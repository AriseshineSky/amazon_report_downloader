        ...
        try:
            ...
            if report == "order_report_today" or report == "all_reports":
                ...
                for i in range(7):
                    ...
                    if (i + index) % 7 == 0:
                        try:
                            status = downloader.go_to_orders_download_page()
                            logger.info(status['message'])
                            if status['status'] == True:
                                downloader.upload_files(seller_profit_domain + "/import_orders", status['report_name'], gideon_email,
            ...
            if report == "advertising_report":
                try:
                    status = downloader.go_to_advertising_reports_download_page()
                    logger.info(status['message'])
                    downloader.upload_files(seller_profit_domain + "/import_ads", status['report_name'], gideon_email,
                                            gideon_password, seller_id, "ads_file", marketplace, seller_profit_domain)
                except Exception as e:
                    print(e)
            ...
            if report == "FBA_inventory_report":
                try:
                    status = downloader.go_to_FBA_inventory_download_page()
                    logger.info(status['message'])
                    if status['status'] == True:
                        downloader.upload_files(seller_profit_domain + "/import_inventory", status['report_name'], gideon_email,
                                                gideon_password, seller_id, "inventory_file", marketplace, seller_profit_domain)
                except Exception as e:
                    print(e)
            ...
            if report == "finance_report":
                try:
                    status = downloader.go_to_finance_download_page()
                    logger.info(status['message'])
                    if status['status'] == True:
                        downloader.upload_files(seller_profit_domain + "/import_finances", status['report_name'], gideon_email,
                                                gideon_password, seller_id, "finances_file", marketplace, seller_profit_domain)
                except Exception as e:
                    print(e)
            ...
            if report == "listings_report":
                try:
                    status = downloader.go_to_listings_download_page()
                    logger.info(status['message'])
                    if status['status'] == True:
                        downloader.upload_files(seller_profit_domain + "/import_listings", status['report_name'], gideon_email,
                                                gideon_password, seller_id, "listings_file", marketplace, seller_profit_domain)
            ...
            if report == "FBA_shipment_report":
                try:
                    status = downloader.go_to_FBA_shipment_download_page()
                    logger.info(status['message'])
                    if status['status'] == True:
                        downloader.upload_files(seller_profit_domain + "/import_orders", status['report_name'], gideon_email,
                                                gideon_password, seller_id, "order_shipments_file", marketplace, seller_profit_domain)
